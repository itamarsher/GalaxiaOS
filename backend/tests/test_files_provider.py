...
@pytest.mark.asyncio
async def test_check_access_is_a_scope_safe_real_call(monkeypatch):
    ...
    # A files.list (NOT a root-metadata GET, which drive.file forbids) — this is the
    # scope-safe call that still forces the token refresh that validates the cred.
    assert captured["method"] == "GET"
    assert captured["url"] == _DRIVE_API  # the files collection, no /<id> suffix
...
async def test_verify_google_drive_does_a_real_reachability_check(monkeypatch):
    ...
    # rejected up front) — i.e. it calls check_access, not the no-op ensure_folder([]).
    from app.services import integrations as integrations_svc
    ...
    monkeypatch.setattr(GoogleDriveFileProvider, "check_access", _fake_check)
...
async def test_verify_google_drive_propagates_failure(monkeypatch):
    ...
    monkeypatch.setattr(GoogleDriveFileProvider, "check_access", _boom)
    with pytest.raises(FileProviderError):
