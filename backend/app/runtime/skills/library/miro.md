...
1. **Find the tool.** `discover_tools` with query `miro`; Miro exposes as `mcp__miro__*` once the
   founder has connected it. Load what you need with `use_tool` (create a board, read frames/items).
2. **Not connected? Ask — don't fake it.** `request_user_action` for the founder to connect Miro in
