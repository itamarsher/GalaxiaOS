...
1. **Find the tool.** `discover_tools` with query `circleci`; it exposes as `mcp__circleci__*` once the
   founder has connected it. Load what you need with `use_tool` (read pipeline status, trigger, get logs).
2. **Not connected? Ask — don't fake it.** `request_user_action` for the founder to connect CircleCI in
