...
1. **Find the tool.** `discover_tools` with query `mixpanel`; it exposes as `mcp__mixpanel__*` once the
   founder has connected it. Load what you need with `use_tool` (query events, funnels, cohorts).
2. **Not connected? Ask — don't fake it.** `request_user_action` for the founder to connect Mixpanel in
