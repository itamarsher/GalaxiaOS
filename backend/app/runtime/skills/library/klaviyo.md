...
1. **Find the tool.** `discover_tools` with query `klaviyo`; it exposes as `mcp__klaviyo__*` once the
   founder has connected it. Load what you need with `use_tool` (read segments, draft a flow, pull metrics).
2. **Not connected? Ask — don't fake it.** `request_user_action` for the founder to connect Klaviyo in
