...
1. **Find the tool.** `discover_tools` with query `kit`; it exposes as `mcp__kit__*` once the founder
   has connected it. Load what you need with `use_tool` (subscribers, sequences, broadcasts).
2. **Not connected? Ask — don't fake it.** `request_user_action` for the founder to connect Kit in
