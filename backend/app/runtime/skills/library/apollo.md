...
1. **Find the tool.** `discover_tools` with query `apollo`; it exposes as `mcp__apollo__*` once the
   founder has connected it. Load what you need with `use_tool` (search people, enrich, add to sequence).
2. **Not connected? Ask — don't fake it.** `request_user_action` for the founder to connect Apollo in
